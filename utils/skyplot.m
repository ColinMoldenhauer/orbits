function hsky = skyplot(azim, elev, line_style, show_direction, DisplayName)
...
if ~hold_state
    ...
    for i=[30 60]
        plot(xunit*i,yunit*i,'-','color',tc,'linewidth',1, 'HandleVisibility', 'off');
    end
    i=90;
    plot(xunit*i,yunit*i,'-','color',tc,'linewidth',2, 'HandleVisibility', 'off');
    for i=[15:30:75 105:15:zenmax]
        plot(xunit*i,yunit*i,':','color',tc,'linewidth',1, 'HandleVisibility', 'off');
    end
    for i=30:30:zenmax
        text(0,i,[' ' num2str(90-i)],'verticalalignment','bottom', 'HandleVisibility', 'off');
   end
...
% plot spokes
   ...
   ca = [-caz; caz];
   sa = [-saz; saz];
   plot(elmax*ca,elmax*sa,'-','color',tc,'linewidth',1, 'HandleVisibility', 'off');
   if zenmax > elmax
      plot(zenmax*ca,zenmax*sa,':','color',tc,'linewidth',1, 'HandleVisibility', 'off');
   end
...
q = plot(xx, yy, line_style, DisplayName=DisplayName);

% include direction of travel
if show_direction
    dx = xx(2:end) - xx(1:end-1);
    dy = yy(2:end) - yy(1:end-1);
    quiv = quiver(xx(1:end-1), yy(1:end-1), dx, dy, HandleVisibility="off");
    quiv.AutoScale = 'off';
end
